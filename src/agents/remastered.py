...
class RemasteredAgent(BaseAgent):
    ...
    def generate_persona(self, state: GlobalState) -> dict[str, Any]:
        ...
        prompt = ChatPromptTemplate.from_messages([
            ("system", "You are an expert product researcher. Create a high-resolution Persona based on the provided business idea."),
    ...
    def generate_alternative_analysis(self, state: GlobalState) -> dict[str, Any]:
        ...
        prompt = ChatPromptTemplate.from_messages([
            ("system", "You are an expert market analyst. Identify current alternative methods (e.g., Excel, existing SaaS) and infer a '10x Value' that outweighs switching costs."),
    ...
    def generate_vpc(self, state: GlobalState) -> dict[str, Any]:
        ...
        prompt = ChatPromptTemplate.from_messages([
            ("system", "You are a product strategist. Create a Value Proposition Canvas validating how 'Pain Relievers' and 'Gain Creators' fit the customer's 'Jobs' and 'Pains/Gains'."),
    ...
    def generate_mental_model_and_journey(self, state: GlobalState) -> dict[str, Any]:
        ...
        if not state.value_proposition or not state.alternative_analysis:
            logger.warning("Missing VPC or Alternative Analysis for Mental Model/Journey generation.")
            return {}

        # 1. Mental Model Diagram
        mm_prompt = ChatPromptTemplate.from_messages([
            ("system", "You are a cognitive psychologist UX researcher. Visualize the 'Towers of Thought' behind the user's actions."),
            ("user", f"VPC Fit: {state.value_proposition.fit_evaluation}\nSwitching Cost: {state.alternative_analysis.switching_cost}\n\nGenerate Mental Model Diagram:")
        ])
        ...
        if isinstance(mm_result, MentalModelDiagram):
            ...
            cj_prompt = ChatPromptTemplate.from_messages([
                ("system", "You are an expert UX researcher. Map chronological actions based on the mental model to a Customer Journey. You must ensure worst_pain_phase perfectly matches one of the phase_names."),
            ...
            try:
                ...
                if isinstance(cj_result, CustomerJourney):
                    return {
                        "mental_model": mm_result,
                        "customer_journey": cj_result
                    }
            except Exception:
    ...
    def generate_sitemap_and_wireframe(self, state: GlobalState) -> dict[str, Any]:
        ...
        prompt = ChatPromptTemplate.from_messages([
            ("system", "You are an Information Architect. Define the entire URL structure and page transition overview of the app, and output a core UserStory targeting the worst pain phase."),
            ("user", f"Worst Pain Phase: {state.customer_journey.worst_pain_phase}\nJourney Phases: {state.customer_journey.model_dump()!s}\n\nGenerate Sitemap and Story:")
        ])
...
class VirtualCustomerAgent(BaseAgent):
    ...
    def run(self, state: GlobalState) -> dict[str, Any]:
        ...
        if not state.sitemap_and_story or not state.target_persona:
            return {}
        prompt = ChatPromptTemplate.from_messages([
            ("system", "You are the Virtual Customer. Provide harsh feedback indicating whether you'd pay for the given user story or if switching costs are too high."),
            ("user", f"Persona: {state.target_persona.model_dump()}\nStory: {state.sitemap_and_story.core_story.model_dump()}\n\nFeedback:")
        ])
        result = (prompt | self.llm).invoke({})
        msg = DialogueMessage(role=Role.NEW_EMPLOYEE, content=str(result.content), timestamp=time.time())
...
class HackerAgent(BaseAgent):
    ...
    def run(self, state: GlobalState) -> dict[str, Any]:
        if not state.sitemap_and_story:
            return {}
        prompt = ChatPromptTemplate.from_messages([
            ("system", "You are the Hacker. Review the wireframe for technical debt, scalability, and security. Avoid unnecessary complexity."),
...
class HipsterAgent(BaseAgent):
    ...
    def run(self, state: GlobalState) -> dict[str, Any]:
        if not state.mental_model or not state.sitemap_and_story:
            return {}
        prompt = ChatPromptTemplate.from_messages([
            ("system", "You are the Hipster. Review the UX based on 'Don't make me think'. Point out friction against the mental model."),
            ("user", f"Mental Model: {state.mental_model.model_dump()}\nStory: {state.sitemap_and_story.core_story.model_dump()}\n\nUX Review:")
        ])
        result = (prompt | self.llm).invoke({})
...
class HustlerAgent(BaseAgent):
    ...
    def run(self, state: GlobalState) -> dict[str, Any]:
        if not state.alternative_analysis or not state.value_proposition:
            return {}
        prompt = ChatPromptTemplate.from_messages([
            ("system", "You are the Hustler. Review the business model unit economics (LTV > 3x CAC). Interrogate who will pay and why."),
            ("user", f"Alternatives: {state.alternative_analysis.model_dump()}\nVPC: {state.value_proposition.model_dump()}\n\nBusiness Review:")
        ])
        result = (prompt | self.llm).invoke({})
...
class OutputGenerationAgent(BaseAgent):
    ...
    def generate_experiment_plan(self, state: GlobalState) -> dict[str, Any]:
        ...
        prompt = ChatPromptTemplate.from_messages([
            ("system", "You are a growth hacker. Generate an Experiment Plan defining 'What and How to measure' using the MVP."),
            ("user", f"Idea: {state.selected_idea.title}\n\nGenerate Experiment Plan:")
        ])
    ...
    def generate_agent_prompt_spec(self, state: GlobalState) -> dict[str, Any]:
        ...
        prompt = ChatPromptTemplate.from_messages([
            ("system", "You are a Senior Technical Lead. Generate a perfect AgentPromptSpec markdown specification for AI coding tools."),
            ("user", f"Core Story: {state.sitemap_and_story.core_story.model_dump()}\n\nGenerate Agent Prompt Spec:")
        ])
