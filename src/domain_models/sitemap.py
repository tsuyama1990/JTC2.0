...
class Route(BaseModel):
    ...
    name: str = Field(
        ..., description="ページ名", min_length=get_settings().validation.min_list_length
    )
    purpose: str = Field(
        ..., description="このページの目的", min_length=get_settings().validation.min_content_length
    )
...
class UserStory(BaseModel):
    model_config = ConfigDict(extra="forbid")
    as_a: str = Field(
        ..., description="誰として(Persona)", min_length=get_settings().validation.min_list_length
    )
    i_want_to: str = Field(
        ...,
    ...
    so_that: str = Field(
        ...,
        description="なぜなら〜を達成したいから(Goal/Value)",
        min_length=get_settings().validation.min_list_length,
    )
...
class SitemapAndStory(BaseModel):
    ...
    sitemap: list[Route] = Field(
        ...,
        description="アプリケーション全体のURL・ルーティング構成",
        min_length=get_settings().validation.min_list_length,
    )
