...
def make_ideator_node(ideator_agent: IAgent, settings: Settings) -> Callable[[GlobalState], dict[str, Any]]:
...
def make_persona_node(agent: IRemasteredAgent, settings: Settings) -> Callable[[GlobalState], dict[str, Any]]:
...
def _alternative_analysis_node_impl(state: GlobalState, agent: IRemasteredAgent, settings: Settings) -> dict[str, Any]:
...
def make_alternative_analysis_node(agent: IRemasteredAgent, settings: Settings) -> Callable[[GlobalState], dict[str, Any]]:
    executor = NodeExecutor(settings, agent=agent, settings_obj=settings)
    # the function definition needs settings_obj mapping
    def wrapper(state: GlobalState, agent: IRemasteredAgent, settings_obj: Settings) -> dict[str, Any]:
...
# VPC
def _vpc_node_impl(state: GlobalState, agent: IRemasteredAgent, settings: Settings) -> dict[str, Any]:
    ...
    if updates.get("value_proposition"):
        ...
        ApprovalStampRenderer("VPC Canvas", settings.simulation).start()
    return updates if isinstance(updates, dict) else {}

def make_vpc_node(agent: IRemasteredAgent, settings: Settings) -> Callable[[GlobalState], dict[str, Any]]:
    executor = NodeExecutor(settings, agent=agent, settings_obj=settings)
    def wrapper(state: GlobalState, agent: IRemasteredAgent, settings_obj: Settings) -> dict[str, Any]:
...
def _mental_model_journey_node_impl(state: GlobalState, agent: IRemasteredAgent, settings: Settings) -> dict[str, Any]:
    ...
    if updates.get("mental_model") or updates.get("customer_journey"):
        ApprovalStampRenderer("Mental Model & Journey", settings.simulation).start()
    return updates if isinstance(updates, dict) else {}

def make_mental_model_journey_node(agent: IRemasteredAgent, settings: Settings) -> Callable[[GlobalState], dict[str, Any]]:
    executor = NodeExecutor(settings, agent=agent, settings_obj=settings)
    def wrapper(state: GlobalState, agent: IRemasteredAgent, settings_obj: Settings) -> dict[str, Any]:
...
# SITEMAP WIREFRAME
def _sitemap_wireframe_node_impl(state: GlobalState, agent: IRemasteredAgent, settings: Settings) -> dict[str, Any]:
    ...
    if updates.get("sitemap_and_story"):
        ...
        ApprovalStampRenderer("Sitemap & Story", settings.simulation).start()
    return updates if isinstance(updates, dict) else {}

def make_sitemap_wireframe_node(agent: IRemasteredAgent, settings: Settings) -> Callable[[GlobalState], dict[str, Any]]:
    executor = NodeExecutor(settings, agent=agent, settings_obj=settings)
    def wrapper(state: GlobalState, agent: IRemasteredAgent, settings_obj: Settings) -> dict[str, Any]:
...
# SPEC GENERATION
def _spec_generation_node_impl(state: GlobalState, agent: IOutputGenerationAgent, settings: Settings) -> dict[str, Any]:
    ...
    if updates.get("agent_prompt_spec"):
        ...
        ApprovalStampRenderer("Agent Prompt Spec", settings.simulation).start()
    return updates if isinstance(updates, dict) else {}

def make_spec_generation_node(agent: IOutputGenerationAgent, settings: Settings) -> Callable[[GlobalState], dict[str, Any]]:
    executor = NodeExecutor(settings, agent=agent, settings_obj=settings)
    def wrapper(state: GlobalState, agent: IOutputGenerationAgent, settings_obj: Settings) -> dict[str, Any]:
...
# EXPERIMENT PLANNING
def _experiment_planning_node_impl(state: GlobalState, agent: IOutputGenerationAgent, settings: Settings) -> dict[str, Any]:
    ...
    if updates.get("experiment_plan"):
        ...
        ApprovalStampRenderer("Experiment Plan", settings.simulation).start()
    return updates if isinstance(updates, dict) else {}

def make_experiment_planning_node(agent: IOutputGenerationAgent, settings: Settings) -> Callable[[GlobalState], dict[str, Any]]:
    executor = NodeExecutor(settings, agent=agent, settings_obj=settings)
    def wrapper(state: GlobalState, agent: IOutputGenerationAgent, settings_obj: Settings) -> dict[str, Any]:
...
def make_virtual_customer_node(agent: IAgent, settings: Settings) -> Callable[[GlobalState], dict[str, Any]]:
    executor = NodeExecutor(settings, agent=agent)
    return lambda state: executor.execute(_virtual_customer_node_impl, state, "Error in Virtual Customer Node")


# 3H REVIEW
def _review_3h_node_impl(state: GlobalState, hacker_agent: IAgent, hipster_agent: IAgent, hustler_agent: IAgent) -> dict[str, Any]:
...
def make_review_3h_node(hacker_agent: IAgent, hipster_agent: IAgent, hustler_agent: IAgent, settings: Settings) -> Callable[[GlobalState], dict[str, Any]]:
    executor = NodeExecutor(settings, hacker_agent=hacker_agent, hipster_agent=hipster_agent, hustler_agent=hustler_agent)
...
def _safe_simulation_run_impl(state: GlobalState, settings: Settings, factory: AgentFactory) -> dict[str, Any]:
...
def make_safe_simulation_run_node(settings: Settings, factory: AgentFactory) -> Callable[[GlobalState], dict[str, Any]]:
    executor = NodeExecutor(settings, settings_obj=settings, factory=factory)
    def wrapper(state: GlobalState, settings_obj: Settings, factory: AgentFactory) -> dict[str, Any]:
...
def _nemawashi_analysis_node_impl(state: GlobalState, engine_factory: Callable[[], NemawashiEngine]) -> dict[str, Any]:
...
def make_nemawashi_analysis_node(engine_factory: Callable[[], NemawashiEngine], settings: Settings) -> Callable[[GlobalState], dict[str, Any]]:
    executor = NodeExecutor(settings, engine_factory=engine_factory)
    return lambda state: executor.execute(_nemawashi_analysis_node_impl, state, "Error in Nemawashi Analysis")
...
def make_governance_node(agent: IAgent, settings: Settings) -> Callable[[GlobalState], dict[str, Any]]:
    executor = NodeExecutor(settings, agent=agent)
