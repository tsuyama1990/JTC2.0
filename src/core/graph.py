...
def create_app() -> CompiledStateGraph:  # type: ignore[type-arg]
    ...
    workflow.add_node("alternative_analysis_node", alternative_analysis_node)
    workflow.add_node("vpc_node", vpc_node)
    ...
    # Phase 2 nodes
    workflow.add_edge("verification", "persona_node")
    workflow.add_edge("persona_node", "alternative_analysis_node")
    workflow.add_edge("alternative_analysis_node", "vpc_node")

    # Gate 1.5 & Gate 2: Customer-Problem Fit (Riskiest Assumption)
    # Interrupt happens after 'vpc_node' returns.
    # User provides feedback or transcript during resume.
    workflow.add_edge("vpc_node", "transcript_ingestion")
    ...
    return workflow.compile(interrupt_after=["ideator", "vpc_node", "solution_proposal", "pmf"])
