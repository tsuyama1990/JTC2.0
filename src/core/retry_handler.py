...
class RetryHandlerConfig:
    def __init__(
        self, logger: logging.Logger | None = None, random_func: Callable[[int], int] | None = None
    ) -> None:
        self.logger = logger or logging.getLogger(__name__)

        self.random_func: Callable[[int], int]
        if random_func is None:
            import secrets

            def _default_random(max_val: int) -> int:
                return secrets.randbelow(max_val)

            self.random_func = _default_random
        else:
