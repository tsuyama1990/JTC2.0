...
def test_config_values() -> None:
    ...
    s = Settings(
        OPENAI_API_KEY=SecretStr("sk-123456789012345678901234567890123456789012345678"),
        TAVILY_API_KEY=SecretStr("tvly-12345678901234567890"),
        V0_API_KEY=SecretStr("v0-123456789012345678901234567890123456789012345678"),
        V0_API_URL="https://api.v0.dev/chat/completions",
    )
    assert s.llm.model == "gpt-4o"
    assert s.search.max_results == 5
    assert s.openai_api_key == SecretStr("sk-123456789012345678901234567890123456789012345678")
