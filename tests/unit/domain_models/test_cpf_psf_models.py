...
def test_customer_journey_valid() -> None:
    phase = JourneyPhase(
        phase_name="Awareness", touchpoint="Ad", customer_action="Clicks",
        mental_tower_ref="Curiosity", pain_points=["Confusing ad"], emotion_score=0
    )
    phase2 = JourneyPhase(
        phase_name="Consideration", touchpoint="Website", customer_action="Reads",
        mental_tower_ref="Research", pain_points=["Too much text"], emotion_score=-1
    )
    phase3 = JourneyPhase(
        phase_name="Purchase", touchpoint="Checkout", customer_action="Buys",
        mental_tower_ref="Commitment", pain_points=["Slow load"], emotion_score=2
    )
...
def test_sitemap_and_story_valid() -> None:
    ...
    story = UserStory(as_a="User", i_want_to="Login", so_that="Access data", acceptance_criteria=["Can login"], target_route="/login")
...
def test_experiment_plan_valid() -> None:
    ...
    plan = ExperimentPlan(
        riskiest_assumption="Users will pay", experiment_type="LP",
        acquisition_channel="Ads", aarrr_metrics=[metric], pivot_condition="<10% conversion"
    )
...
def test_agent_prompt_spec_valid() -> None:
    ...
    spec = AgentPromptSpec(
        sitemap="Paths", routing_and_constraints="Next.js", core_user_story=story,
        state_machine=sm, validation_rules="Zod", mermaid_flowchart="graph TD"
    )
