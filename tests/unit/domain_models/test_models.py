...
def test_metrics_limit_custom() -> None:
    ...
    # Create a dict with 11 entries (max is 10)
    excessive_metrics = {
        f"metric_{i}": float(i) for i in range(11)
    }
...
def test_global_state_lifecycle_validation() -> None:
    ...
    assert "Target persona is required for the Verification phase." in str(exc.value)
    ...
    with pytest.raises(ValidationError) as exc:
        GlobalState.model_validate(state.model_dump())
    assert "MVP Definition is required for the Solution phase." in str(exc.value)
