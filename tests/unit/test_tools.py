...
@patch("src.adapters.search_adapter.TavilyClient")
@patch("src.adapters.search_adapter.get_settings")
def test_tavily_search_success(mock_get_settings: MagicMock, mock_client_cls: MagicMock) -> None:
    ...
    mock_client.search.assert_called_with(query="query", max_results=5, search_depth="advanced", include_raw_content=False)
...
def test_tavily_safe_search_error(mock_get_settings: MagicMock, mock_client_cls: MagicMock) -> None:
    ...
    # safe_search should catch the error and return error message
    result = search.safe_search("query")
