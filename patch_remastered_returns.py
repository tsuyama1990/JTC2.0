...
with open("src/agents/remastered.py") as f:
    content = f.read()
...
<<<<<<< SEARCH
    def generate_alternative_analysis(self, state: GlobalState) -> dict[str, Any]:
        ...
        if not state.target_persona or not state.selected_idea:
            logger.warning("Missing persona or idea for Alternative Analysis.")
            return {}
=======
    def generate_alternative_analysis(self, state: GlobalState) -> dict[str, Any]:
        \"\"\"Generate Alternative Analysis.\"\"\"
        if not state.target_persona or not state.selected_idea:
            logger.warning("Missing persona or idea for Alternative Analysis.")
            raise ValueError("Missing persona or idea for Alternative Analysis.")
>>>>>>> REPLACE
<<<<<<< SEARCH
    def generate_vpc(self, state: GlobalState) -> dict[str, Any]:
        \"\"\"Generate Value Proposition Canvas.\"\"\"
        if not state.target_persona or not state.selected_idea:
            logger.warning("Missing persona or idea for VPC generation.")
            return {}
=======
    def generate_vpc(self, state: GlobalState) -> dict[str, Any]:
        \"\"\"Generate Value Proposition Canvas.\"\"\"
        if not state.target_persona or not state.selected_idea:
            logger.warning("Missing persona or idea for VPC generation.")
            raise ValueError("Missing persona or idea for VPC generation.")
>>>>>>> REPLACE
<<<<<<< SEARCH
...
with open("patch_remastered_returns.patch", "w") as f:
    f.write(diff)
