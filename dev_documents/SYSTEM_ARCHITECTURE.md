...
### 3.1. LLM Self-Correction Loop & Resilience

A critical component of this architecture is the **Anti-Hallucination Self-Correction Loop**. Due to the strict enforcement of Pydantic schemas (with `extra="forbid"`), any deviation or hallucinated field generated by the LLM will raise a `pydantic.ValidationError`.
Rather than crashing, the architecture mandates wrapping all LLM structured output generations in a `tenacity` `@retry` block. If a validation error occurs, the exception message—detailing exactly which fields are missing or invalid—is captured and fed back into the prompt dynamically on the subsequent retry attempt. This creates a fail-fast, self-healing loop that guarantees robust data pipelines across nodes.

### 3.2. Thread Safety & UI Decoupling

The JTC 2.0 system integrates a headless background orchestrator (LangGraph) with a rigid frontend game engine (Pyxel). Since Pyxel strictly demands control over the main thread and its own update/draw loop, direct synchronous calls to LangGraph nodes would block the UI and cause the application to freeze.
To resolve this safely, the system uses a **decoupled threading model**:
- **Background Thread**: The LangGraph simulation execution runs asynchronously (or in a dedicated Python `threading.Thread`), sequentially updating a thread-safe singleton or shared memory structure `GlobalState`.
- **Main Thread**: The Pyxel engine runs exclusively on the main thread. It uses a `state_getter` callback to periodically poll the shared `GlobalState` during its `update()` and `draw()` cycles, rendering animations (like the "Approval" stamp) reactively based on phase transitions without risking race conditions.
...
The project is divided into **6 Sequential Cycles** corresponding to the 14-Step "Fitness Journey Workflow" outlined in the specifications.

### 5.1. 14-Step Workflow to Cycle Mapping

| Phase / Step | Name | Associated Implementation Cycle |
| :--- | :--- | :--- |
| **Phase 1: Idea Verification** | | |
| Step 1 | Ideation & PEST Analysis (`ideator_node`) | **CYCLE 01** |
| *HITL Gate 1* | Select "Plan A" | **CYCLE 01** |
| **Phase 2: Customer / Problem Fit** | | |
| Step 2 | Persona & Empathy Mapping | **CYCLE 01** / **CYCLE 02** |
| Step 3 | Alternative Analysis | **CYCLE 02** |
| Step 4 | Value Proposition Design | **CYCLE 02** |
| *HITL Gate 1.5* | CPF Feedback | **CYCLE 02** |
| Step 5 | Problem Interview RAG (`transcript_ingestion`) | **CYCLE 03** |
| **Phase 3: Problem / Solution Fit**| | |
| Step 6 | Mental Model & Journey Mapping | **CYCLE 03** |
| Step 7 | Sitemap & Lo-Fi Wireframing | **CYCLE 03** |
| *HITL Gate 1.8* | PSF Feedback | **CYCLE 03** |
| **Phase 4: Validation & Review** | | |
| Step 8 | Virtual Solution Interview | **CYCLE 04** |
| *HITL Gate 2* | Pivot / Proceed Decision | **CYCLE 04** |
| Step 9 | JTC Board Simulation (Gekizume) | **CYCLE 02** / **CYCLE 04** |
| Step 10 | 3H Review | **CYCLE 04** |
| **Phase 5: Output Generation** | | |
| Step 11 | Agent Prompt Spec Generation | **CYCLE 05** |
| Step 12 | Experiment Planning | **CYCLE 05** |
| *HITL Gate 3* | Final Output Feedback | **CYCLE 05** |
| **Phase 6: Governance** | | |
| Step 13 | Governance Check (Ringi-sho) | **CYCLE 06** |
| Step 14 | Final Artifact Generation | **CYCLE 06** |
