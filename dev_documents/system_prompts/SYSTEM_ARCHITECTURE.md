...
To manage the psychological aspect of the simulation and provide dopamine-driven feedback, we update the Pyxel interface in `src/ui/stamps.py`. We will implement a visual rendering routine that draws a large, red "Approval" (承認) hanko stamp animation on the screen. To prevent threading race conditions between the asynchronous LangGraph backend and the synchronous Pyxel rendering loop, the UI will strictly employ a polling mechanism. The renderer will observe specific boolean flags (e.g., `cpf_validated`) within the `GlobalState` object. When a flag transitions to true following a successful resume from Gate 1.5, the animation triggers locally on the UI thread, gamifying the tedious business validation process and providing the user with immediate, satisfying visual confirmation that the CPF phase is complete.
...
Following the JTC board, we implement the entirely new `3h_review_node`. This utilizes a parallel processing pattern (or a sequential sub-graph, depending on LangGraph constraints) to invoke three distinct personas simultaneously: The Hacker, The Hipster, and The Hustler. The Hacker evaluates the `SitemapAndStory` for technical debt, security risks, and unnecessary architectural complexity. The Hipster evaluates the UX flow against the `CustomerJourney` for cognitive friction and accessibility. The Hustler evaluates the unit economics against the `AlternativeAnalysis`. Their combined, highly specialized critiques are appended as `DialogueMessage` objects to the `debate_history` array in the `GlobalState`, creating a comprehensive, multi-faceted audit trail of the product's viability prior to final spec generation. To guarantee deterministic execution during this adversarial phase, a strict moderator/synthesizer pattern must be implemented within this cycle to forcibly resolve conflicting advice (e.g., the Hacker wanting a simpler architecture versus the Hipster demanding complex animations) before proceeding. Furthermore, hard circuit breaker limits (`max_turns`) must be implemented natively within these review nodes during this cycle to definitively prevent infinite agent debates and token exhaustion.
...
The `3h_review_node` presents a significant testing challenge due to its complex adversarial dynamics. We will write unit tests that deliberately force the mocked Hacker, Hipster, and Hustler agents to return highly conflicting advice (e.g., mutually exclusive architectural recommendations) that repeatedly triggers the internal debate logic. We will configure a test scenario where the agents fail to reach consensus within the explicitly implemented `max_turns` limit. The absolute critical assertion for this test is that the cycle's specific circuit breaker and synthesizer logic successfully activates, forcefully terminating the debate, logging a warning, resolving the conflict deterministically, and returning a safe, stable fallback state rather than causing a stack overflow or exhausting the mocked API rate limits.
